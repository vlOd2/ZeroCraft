...
public class FilledPacket {
	...
	public byte[] packageToByteArray() {
		...
		for (int packetFieldIndex = 0; 
            ...
            if (packetField == byte.class) {
            	...
            	data = ByteBuffer.allocate(4).order(ByteOrder.BIG_ENDIAN).putInt((int)packetFieldValue).array();
	...
	public static FilledPacket unpackageFromByteArray(byte arg0, byte[] arg1) {
		if (!Packets.isValidPacketID(arg0)) throw new IllegalArgumentException();
	        ...
	        if (packet.packetFields.length >= 1) {
	            for (int packetFieldIndex = 0;
	                ...
	                if (packetField == byte.class) {
	                	...
	                	packetData.add(ByteBuffer.wrap(rawShortVal).order(ByteOrder.BIG_ENDIAN).getInt());
	                    currentDataIndex += 4;
	                } else if (packetField == String.class) {
